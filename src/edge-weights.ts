 ...
 * `src/representation/styles.ts` alongside DOM-touching style helpers
 * (`resolveStyleColor` reads `document`/`getComputedStyle`), so that whole
 * module can't move into this runtime-agnostic engine package. These two
 * exports are extracted here **verbatim** (byte-identical values/logic, no
 * behavior change) since `graph.ts` — a slice-1 module — depends on them at
 * runtime. `representation/styles.ts` itself stays in template for now (its
